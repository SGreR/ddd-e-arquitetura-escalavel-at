...
public class MessageConfig {
    ...
    public PubSubInboundChannelAdapter almoxarifadoChannelAdapter(
        ...
        PubSubInboundChannelAdapter adapter = new PubSubInboundChannelAdapter(pubSubTemplate, "PetFriends_Produtos_Pedidos");
    ...
    @Bean
    public MessageChannel entregaMessageChannel() {
        return new PublishSubscribeChannel();
    }

    @Bean
    public PubSubInboundChannelAdapter entregaChannelAdapter(
            @Qualifier("entregaMessageChannel") MessageChannel messageChannel, PubSubTemplate pubSubTemplate){
        pubSubTemplate.setMessageConverter(statusMudouConverter());
        PubSubInboundChannelAdapter adapter = new PubSubInboundChannelAdapter(pubSubTemplate, "PetFriends_Entrega_Pedidos");
        adapter.setOutputChannel(messageChannel);
        adapter.setAckMode(AckMode.MANUAL);
        adapter.setPayloadType(StatusPedidoMudou.class);
        return adapter;
